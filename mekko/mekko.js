...
d3.mekko = function() {
  var width = 1,
      ...
      p = d3.format("%"),
      value = mekkoValue,
      categoryX = mekkoCategoryX,
      categoryY = mekkoCategoryY,
      duration = 0;
  ...
  function mekko(g) {
    g.each(function(data, i) {
      ...
      var groups = d3.nest()
          .key(categoryX)
          .rollup(function(d) { return d.map(mekkoData); })
          .entries(data);
...
function mekkoValue(d) {
  return d.value;
}

function mekkoCategoryX(d) {
  return d.x;
}

function mekkoCategoryY(d) {
  return d.y;
}

function mekkoData(d) {
  return {data: d};
}
